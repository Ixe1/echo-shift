...
- For boss gameplay/render changes, cover every affected combat phase in simulation and browser QA; do not rely only on the opening attack if later phases change.
...
- Diagnostics exposed on `document.documentElement.dataset` are used by Playwright QA. Update tests when diagnostics change, and prefer diagnostics-guided browser routes over brittle fixed millisecond input timing.
