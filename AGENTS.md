...
Do not add, maintain, or run route/playthrough QA. Avoid automated tests that drive a scripted completion path, boss route, jump route, or level-clear route through browser input. Prefer deterministic simulation assertions, direct state/diagnostic checks, screenshots, and manual play inspection for gameplay feel.
...
- For rendered gameplay, sprites, structural outlines, doors, sensors, or visual diagnostics, run focused non-route visual checks such as `npm run qa:door-solid` when appropriate.
- For boss gameplay/render changes, cover affected combat logic in simulation and use focused diagnostics, screenshots, or manual inspection for visuals. Do not create browser route QA to reach later phases.
...
- Boss vulnerable-window misses are real gameplay states. Test ascent/descent transitions after the player does not damage the boss, not only after successful hits.
- Lane-based boss attacks should resolve warnings, impacts, floor effects, and damage from the lane or projectile position, not from one arena-wide floor sample.
...
- Diagnostics exposed on `document.documentElement.dataset` are used by Playwright QA. Update focused diagnostics when they change, but do not build diagnostics-guided browser routes or scripted playthrough tests.
